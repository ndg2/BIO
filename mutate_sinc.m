...
function newIndividual = mutate_sinc(population,nHiddenLayer,sortedIndices)
...
if rand(1)>0.5 % optimize bias
    ...
    if strcmp(class,'good')
        noise = 0.05*(rand(length(column),1)-0.5);
    else if strcmp(class,'medium')
            noise = 0.1*(rand(length(column),1)-0.5);
        else
            noise = 0.2*(rand(length(column),1)-0.5);
        end
    end
    column = column+noise;
