...
% ANN Structure
nHiddenLayer = 4; % Number of Hidden Layers
HiddenNeurons = [35;35;35;35]; % Number of neurons on each Hidden Layer
% Parameters
maxiter = 4999; % Maximum number of iterations for GA
dim = 2; % Dimension of input
maxValue = 50;
range = [-5 5];
nPairs = 200; % Number of input-output pairs
nIndividuals = 200; % Number of individuals in population
...
[inputs,DesiredOutputs] = GenerateTrainingSetSphere(dim,nPairs, range);
...
for counter = 1:maxiter
    ...
    for index = 1:(nCrossover + nMutation)
        ...
        if index <= nCrossover
            newIndividuals{index} = crossover_sphere(subsetPopulation,nHiddenLayer);
        else
            newIndividuals{index} = mutate_sphere(population,nHiddenLayer,SortedIndexes);
        end
    end
...
RealOutput = zeros(1,nPairs);
for i = 1:nPairs
    output = 0;
    for j = 1:dim
        output = output + inputs(i,j).^2;
    end
    RealOutput(i) = output;
end
